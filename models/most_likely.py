...
data_2025 = pd.read_excel('/Users/bhaskaravanacharla/Downloads/Documents/Machine_learning/admissionnn/fall25/Fall 2025 5.5.25_processed_norm.xlsx')
...
# Assuming you already have 'predictions' and 'probabilities' from the model
data_2025['predicted_admitted'] = predictions

# Move probabilities to CPU and then convert to numpy
probabilities_cpu = probabilities.cpu().numpy()  # Ensure you're on CPU before converting

# Add the probabilities as a new column
data_2025['probabilities'] = probabilities_cpu[:, 1]  # Extract probabilities for class 1 (admitted)
...
admitted_applications_sorted = admitted_applications.sort_values(by='probabilities', ascending=False)

# Select the relevant columns: Application Reference ID, probabilities, and actual admission label (admitted)
output_data = admitted_applications_sorted[['Application Reference ID', 'probabilities', 'admitted']]

# Print the output data (Application ID, probabilities, actual label)
print(output_data)

# Optionally, save the output data to an Excel file
output_data.to_excel('/Users/bhaskaravanacharla/Downloads/Documents/Machine_learning/admissionnn/models/admitted_application_data_sorted.xlsx', index=False)
